...
namespace TOME.UI
{
    public class CollectionBookUI : MonoBehaviour
    {
        ...
        private void Awake()
        {
            // Resources/Recipes 폴더의 모든 레시피 자동 로드 (인스펙터가 비어있을 경우)
            if (recipes == null || recipes.Length == 0)
            {
                recipes = Resources.LoadAll<RecipeSO>("Recipes");
            }
        ...
        private void ShowUnknown()
        {
            if (unknownPanel != null)
            {
                ...
                {
                    unknownMessageText.text = "아직 발견하지 못한 조합 캐릭터예요!";
                }
        ...
        private void ShowDetail(RecipeSO recipe)
        {
            ...
            if (detailAbilityText != null)
            {
                ...
                {
                    abilityDesc = "특별한 패시브 능력이 없는 일반 캐릭터입니다.";
                }
