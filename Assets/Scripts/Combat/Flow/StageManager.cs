...
namespace TOME.Combat
{
    ...
    public class StageManager : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            // 튜토리얼용 초반 지급 아이템 (GameManager.starterItems — 임시 코드, 추후 삭제 예정)
        ...
        void OnFinished(bool win)
        {
            ...
            if (win && node != null)
            {
                // 승리 1회 = 카운트 1 적립. 요구 횟수(기획서: 스테이지별 1~4)를 다 채워야
                // '모두 클리어'로 인정되어 다음 노드가 개방된다.
                int count = _stage != null && TOME.Save.SaveSystemManager.I != null
                    ? TOME.Save.SaveSystemManager.I.IncrementStageClearCount(_stage.id)
                    : int.MaxValue;   // 세이브 없는 단독 실행 테스트에서는 기존처럼 즉시 클리어
                if (_stage == null || count >= _stage.clearRequirement)
                {
