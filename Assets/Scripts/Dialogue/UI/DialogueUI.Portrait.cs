...
namespace TOME.Dialogue
{
    ...
    public partial class DialogueUI
    {
        ...
        [Tooltip("NPC(유령) 초상화 루트. 유령이 처음 화자가 되기 전까진 비활성. 화자 판단은 speaker가 npcSpeakerKeyword 포함 여부.")]
        [SerializeField] GameObject npcPortraitRoot;
        [SerializeField] RawImage   npcPortraitImage;
        [Tooltip("speaker에 이 문자열이 들어 있으면 NPC가 말하는 줄로 본다.")]
        [SerializeField] string     npcSpeakerKeyword = "유령";
        ...
        /// <summary>Awake에서 호출. 프레임 로드 + 초상화 초기 상태.</summary>
        void InitPortrait()
        {
        ...
        /// <summary>OnLine에서 호출. 화자(NPC 여부) 판정 + 좌우 초상화 표시·회색 처리.</summary>
        void UpdateSpeakerSides(string speaker)
        {
            _npcSpeaking = !string.IsNullOrEmpty(speaker) && !string.IsNullOrEmpty(npcSpeakerKeyword)
                        && speaker.Contains(npcSpeakerKeyword);
        ...
        /// <summary>OnEnd에서 호출. NPC 표시·색·입 상태를 세션 시작 전으로 되돌린다.</summary>
        void ResetPortraitOnEnd()
        {
            _npcAppeared = false;
            _npcSpeaking = false;
